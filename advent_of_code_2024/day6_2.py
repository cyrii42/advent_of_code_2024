...
class Guard():
    ...
    total_loops_found: int = field(default=0, repr=False)
    ...
    def find_next_point(self) -> Point:
        ...
        if isinstance(next_point, Obstacle):
            ...
            if self.just_avoided_obstacle:
                print(f"Found a loop!")
                self.total_loops_found += 1
                return 1
            else:
                self.just_avoided_obstacle = True
                self.rotate()
                return self.find_next_point()
        else:
            self.just_avoided_obstacle = False
...
def find_loops(first_guard: Guard, points_to_check: list[Point]) -> int:
    ...
    for visited_point in points_to_check:
        ...
        new_map = Map(first_guard.map.row_list)
        ...
        # print(new_map)
        total += new_guard.patrol_part_two()
    return total
...
def part_two(filename: Path) -> int:
    ...
    points_to_check = [point for point in guard.positions_visited if not isinstance(point, Obstacle) and not isinstance(point, MapEdge) and point.char not in ['^', '>', '<', 'v']]
